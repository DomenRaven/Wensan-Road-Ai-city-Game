...
def skill_ids(case: dict[str, Any]) -> list[str]:
    answers: dict[str, Any] = dict(case.get("creative_answers", {}))
    raw: Any = answers.get("q_skill")
...
def run_slug_playwright(
    ...
    *,
    ...
    skills: list[str] = skill_ids(case)
    gen_ok: bool = False
    ...
    with sync_playwright() as pw:
        ...
        page.evaluate(
            """
            (payload) => {
              const { answers, skills } = payload;
              for (const [qid, value] of Object.entries(answers)) {
                if (qid === "q_skill") continue;
              ...
              for (const sid of skills) {
                document.querySelector(`.skill-chip[data-skill="${sid}"]`)?.click();
              }
            }
            """,
            {"answers": answers, "skills": skills},
        )
    ...
    return {
        ...
        "b4_skill_count": len(skills),
        "fighting_no_skill": slug == "fighting" and len(skills) == 0,
    }
...
def run_smoke(
    ...
    *,
    ...
    if mode == "http":
        print(
            "⚠ Playwright 不可用 · 降级 HTTP API 链 · "
            "须附手工浏览器 checklist（≥3 款）方可过关 #6"
        )
