...
MANIFEST_PATH: Path = REPO_ROOT / "config" / "frozen_genre_parkour.json"
SNAPSHOT_ROOT: Path = (
    REPO_ROOT / "开发文档" / "模板引擎" / "快照" / "frozen" / "6.24_parkour_v1.0"
)

GENRE: str = "parkour"
EDU_HOOKS: tuple[str, ...] = ("parkour_hooks.gd",)
ANCHOR_FILES: tuple[str, ...] = (
    "config/code_anchors/parkour.json",
    "config/creative_templates/parkour.json",
)
...
def build_manifest() -> dict[str, object]:
    ...
    return {
        ...
        "shared_association": [
            "kiosk/edu/edu-wizard.js · getGenreHighlightFallback().parkour",
            "kiosk/edu/edu-wizard.js · renderB7DemoActionsHtml parkour 分支",
        ],
        "files": files,
    }
...
def verify_manifest() -> int:
    ...
    print(f"OK · {count} frozen parkour files match manifest")
    return 0
...
def main() -> int:
    ...
    print("Usage: freeze_parkour_snapshot.py [write|verify]")
    return 2
