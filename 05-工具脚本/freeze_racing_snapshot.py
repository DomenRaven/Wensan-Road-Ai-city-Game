...
MANIFEST_PATH: Path = REPO_ROOT / "config" / "frozen_genre_racing.json"
SNAPSHOT_ROOT: Path = (
    REPO_ROOT / "开发文档" / "模板引擎" / "快照" / "frozen" / "6.24_racing_v1.0"
)

GENRE: str = "racing"
EDU_HOOKS: tuple[str, ...] = ("racing_hooks.gd",)
ANCHOR_FILES: tuple[str, ...] = (
    "config/code_anchors/racing.json",
    "config/creative_templates/racing.json",
)
...
def build_manifest() -> dict[str, object]:
    ...
    return {
        ...
        "shared_association": [
            "kiosk/edu/edu-wizard.js · getGenreHighlightFallback().racing",
            "kiosk/edu/edu-wizard.js · renderB7DemoActionsHtml racing 分支",
        ],
        "files": files,
    }
...
def verify_manifest() -> int:
    ...
    print(f"OK · {count} frozen racing files match manifest")
    return 0
...
def main() -> int:
    ...
    print("Usage: freeze_racing_snapshot.py [write|verify]")
    return 2
