...
MANIFEST_PATH: Path = REPO_ROOT / "config" / "frozen_genre_fighting.json"
SNAPSHOT_ROOT: Path = (
    REPO_ROOT / "开发文档" / "模板引擎" / "快照" / "frozen" / "6.24_fighting_v1.0"
)

GENRE: str = "fighting"
EDU_HOOKS: tuple[str, ...] = ("fighting_hooks.gd", "fighting_touch_overlay.gd")
ANCHOR_FILES: tuple[str, ...] = (
    "config/code_anchors/fighting.json",
    "config/creative_templates/fighting.json",
)
...
def collect_frozen_paths() -> list[Path]:
    paths: list[Path] = _collect_template_files(GENRE)
...
def build_manifest() -> dict[str, object]:
    ...
    return {
        ...
        "shared_association": [
            "kiosk/edu/edu-wizard.js · getGenreHighlightFallback().fighting",
            "kiosk/edu/edu-wizard.js · renderB7DemoActionsHtml fighting 分支",
            "backend/app/services/edu_workspace.py · fighting FightingTouch 注入",
        ],
        "files": files,
    }
...
def verify_manifest() -> int:
    ...
    print(f"OK · {count} frozen fighting files match manifest")
    return 0
...
def main() -> int:
    ...
    print("Usage: freeze_fighting_snapshot.py [write|verify]")
    return 2
