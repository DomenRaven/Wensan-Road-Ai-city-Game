...
| `05-工具脚本/` | E2E、冻结快照、Redis 安装、**一键启动**（`launch_workshop.py` / `启动游戏工坊.exe`） |
...
| fighting | 格斗对战 | `templates/fighting/` |
| parkour | 跑酷 | `templates/parkour/` |
| racing | 欢乐赛车 | `templates/racing/` |

每款含：`core/`（预制逻辑，不可改）· `config/game_config.json`（问卷写入 workspace）· `templates/_edu/{slug}_hooks.gd`（B7 动作上报，七款共用 `_edu` 目录）。
...
|------|------|
| **本文** | 总体方案 · 功能 · 架构 · 流程 |
| [`部署手册_v1.1.md`](./部署手册_v1.1.md) | 环境安装 · 一键启动 exe · 展厅手动启动 |
