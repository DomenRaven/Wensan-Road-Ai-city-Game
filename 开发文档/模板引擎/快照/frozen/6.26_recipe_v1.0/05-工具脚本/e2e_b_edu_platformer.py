...
def run_e2e_b_edu(base: str, display_name: str = "星星大冒险") -> dict[str, Any]:
    ...
    lives: dict[str, Any] = tuning.get("lives", {})
    scoring: dict[str, Any] = tuning.get("scoring", {})
    ...
    assert_ok(
        "E10_lives_max",
        lives.get("max") == EXPECTED_TUNING["lives.max"],
        str(lives.get("max")),
    )
    assert_ok(
        "E10_coin_score",
        scoring.get("coin") == EXPECTED_TUNING["scoring.coin"],
        str(scoring.get("coin")),
    )
    assert_ok(
        "E10_display_name",
    ...
    results["assertions"] = {
        ...
        "E10": (
            ...
            and lives.get("max") == EXPECTED_TUNING["lives.max"]
            and scoring.get("coin") == EXPECTED_TUNING["scoring.coin"]
        ),
        "E11": edu_applied,
