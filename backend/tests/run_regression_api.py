...
def _run_checks(client: TestClient, passed_holder: list[int]) -> None:
    ...
    if r.status_code != 404:
        fail(f"missing shmup template should 404, got {r.status_code}")
    ok("GET /creative/templates/shmup -> 404 (P1 not ready)")
    passed += 1

    answers = {"q_move": "fast", "q_jump": "high", "q_enemy": "hard", "q_skill": "double_jump"}
