...
func get_display_name() -> String:
	...
	return str(meta.get("display_name", "横版闯关"))
...
func _clamp_tuning() -> void:
	...
	if tuning.has("player") and tuning["player"] is Dictionary:
		...
		player["move_speed"] = TuningClampUtil.clamp_float(float(player.get("move_speed", 220)), 220.0)
		player["jump_velocity"] = TuningClampUtil.clamp_negative_magnitude(
			float(player.get("jump_velocity", -514)), -514.0, 0.3
		)
		player["max_fall_speed"] = TuningClampUtil.clamp_float(float(player.get("max_fall_speed", 650)), 650.0)
