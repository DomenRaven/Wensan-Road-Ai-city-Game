...
func get_display_name() -> String:
	...
	return str(meta.get("display_name", "糖果幸存者"))
...
func _clamp_tuning() -> void:
	...
	if tuning.has("player") and tuning["player"] is Dictionary:
		var player: Dictionary = tuning["player"] as Dictionary
		player["speed"] = TuningClampUtil.clamp_float(float(player.get("speed", 150)), 150.0)
		player["max_hp"] = TuningClampUtil.clamp_int(int(player.get("max_hp", 100)), 100)
		tuning["player"] = player
	if tuning.has("spawn") and tuning["spawn"] is Dictionary:
		var spawn: Dictionary = tuning["spawn"] as Dictionary
		if spawn.has("interval_ms"):
			spawn["interval_ms"] = TuningClampUtil.clamp_float(float(spawn.get("interval_ms", 4000)), 4000.0)
		elif spawn.has("rate_per_sec"):
			spawn["rate_per_sec"] = TuningClampUtil.clamp_float(float(spawn.get("rate_per_sec", 0.25)), 0.25)
		tuning["spawn"] = spawn
