...
const TuningClampUtil := preload("res://core/tuning_clamp.gd")
...
func get_display_name() -> String:
	...
	return str(meta.get("display_name", "花园守卫战"))
...
func _clamp_tuning() -> void:
	if not config.has("tuning") or not config["tuning"] is Dictionary:
		return
	var tuning: Dictionary = config["tuning"] as Dictionary
	if tuning.has("economy") and tuning["economy"] is Dictionary:
		var economy: Dictionary = tuning["economy"] as Dictionary
		economy["start_gold"] = TuningClampUtil.clamp_int(int(economy.get("start_gold", 120)), 120)
		tuning["economy"] = economy
	if tuning.has("tower") and tuning["tower"] is Dictionary:
		var tower: Dictionary = tuning["tower"] as Dictionary
		if tower.has("basic") and tower["basic"] is Dictionary:
			var basic: Dictionary = tower["basic"] as Dictionary
			basic["damage"] = TuningClampUtil.clamp_int(int(basic.get("damage", 15)), 15)
			basic["cost"] = TuningClampUtil.clamp_int(int(basic.get("cost", 50)), 50)
			basic["range"] = TuningClampUtil.clamp_int(int(basic.get("range", 3)), 3)
			tower["basic"] = basic
		tuning["tower"] = tower
