...
func _read_run_stats() -> Dictionary:
	if _manager == null:
		return {"score": 0, "survival_ms": 0, "metric": "distance_m"}
	var inst = _manager.get("_game_instance")
	if inst != null and is_instance_valid(inst):
		var dist: int = 0
		var sec: int = 0
		if inst.has_method("get_distance_meters"):
			dist = int(inst.call("get_distance_meters"))
		if inst.has_method("get_survival_seconds"):
			sec = int(inst.call("get_survival_seconds"))
		return {"score": dist, "survival_ms": sec * 1000, "metric": "distance_m"}
	return {"score": 0, "survival_ms": 0, "metric": "distance_m"}
