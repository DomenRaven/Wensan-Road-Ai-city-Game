...
func get_core() -> Dictionary:
	var merged: Dictionary = {}
	for key: String in ["task", "interaction", "economy", "fail"]:
		if config.has(key) and config[key] is Dictionary:
			merged.merge(config[key] as Dictionary)
	return merged
...
func get_display_name() -> String:
	...
	return str(meta.get("display_name", "美味小厨房"))
